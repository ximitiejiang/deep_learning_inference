...
3. 访问权限控制：(CP-240)
- public：对所有对象开放接口
- private：对友元函数、友元类、派生类开放接口，但对其他对象不开放
- protect：对友元函数、友元类、派生类开放接口，

4. 友元的定义：用来给其他类和对象访问private数据提供通道，可以定义友元函数和友元类
- 某个类的友元函数，需要在类的成员函数声明处声明一个友元函数，而对该外部函数则完全没有影响(相当于改了锁，用户就不用改了)
- 友元函数：能够访问原类的私有成员。
```
...
5. 如果某个类不希望被继承，则采用final关键字在类名后边。
```
class AAA final{
};
```
...
```
- 虚函数的执行过程：这是多态实现的底层机理(CP-530)
首先是应用函数的形参必须是基类的指针或引用，然后应用函数调用时形参可以传入一个基类，也可传入一个派生类。如果传入派生类则会根据形参类型进行派生类到基类的类型转换。
